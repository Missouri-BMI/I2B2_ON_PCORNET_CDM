...
CREATE OR REPLACE TABLE {metadata_schema}.ADI_RANKING (
...
insert into {metadata_schema}.ADI_RANKING (C_HLEVEL, C_FULLNAME, C_NAME, C_SYNONYM_CD, C_VISUALATTRIBUTES, C_TOTALNUM, C_BASECODE,
...
with state_rank as (
    select distinct ADI_STATERNK as ADI_STATERNK from {source_schema}.DEID_ADI
    where adi_staternk is not null
    order by ADI_STATERNK
), 
national_rank as (
    select distinct ADI_NATRANK as ADI_NATRANK from {source_schema}.DEID_ADI
...
select 
    ...
    , 'MU'
    , NULL
FROM {metadata_schema}.ADI_RANKING
