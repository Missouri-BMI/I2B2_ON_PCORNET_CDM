...
with DAG(
    ...
    INIT_SCRIPT_PATH = f"{BASE_PATH}/CONFIGURE/COMMON/init.sql"
    CLEANUP_SCRIPT_PATH = f"{BASE_PATH}/CONFIGURE/COMMON/cleanup.sql"
    PROJECT_CONFIGURE_PATH = f"{BASE_PATH}/CONFIGURE/{project}/project_configure.sql"
    ...
    with TaskGroup('init') as init:
        read_sql = read_sql_from_file(INIT_SCRIPT_PATH, **kwargs)
        create_tables_task = execute_sql(
            snowflake_conn_id, 
            get_task_id(INIT_SCRIPT_PATH),
            read_sql
        )
...
# // enable SAML
            ...
            clean_proc_task = execute_sql(
                snowflake_conn_id, 
                get_task_id(CLEANUP_SCRIPT_PATH),
                read_sql
            )
        ...
        with TaskGroup('i2b2-project-configure') as i2b2projectconfigure:
            ...
            proj_config_task = execute_sql(
                snowflake_conn_id, 
                get_task_id(PROJECT_CONFIGURE_PATH),
                read_sql
            )
            read_sql >> proj_config_task  
