...
$$
    ...
    BEGIN
        ...
        /* rename project */
        set project_text = 'GPC Data Lake';

        set harvest = $source_schema || '.' || 'HARVEST';
        select TO_CHAR(MAX(REFRESH_ENCOUNTER_DATE), 'MMMM YYYY') into :date_text from identifier($harvest)
        where datamartid = 'C4UMO';
        set display_text = $project_text || ' (' || :date_text || ')';
